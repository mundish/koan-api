...
The above is generated by Gemini 2.0.
https://gemini.google.com/app/5f5c3a7628b27e64?hl=sv

---

## Environment Variables Usage by File

### Current Files

#### `db/prisma.config.ts`

- **DATABASE_URL** - Used for Prisma migrations and schema management

#### `db/prisma/seed.ts`

- **DATABASE_URL** - Used to connect to database for seeding

#### `packages/services/KoanService.ts` (Phase 2, Step 1)

- **DATABASE_URL** - Used to initialize PrismaClient for database operations

### Future Files (Planned)

#### `apps/express-api/src/index.ts` (Phase 2, Step 2)

- **DATABASE_URL** - Indirectly via KoanService
- **EXPRESS_API_PORT** - Server port configuration
- **NODE_ENV** - Environment mode (development/production)

#### `apps/hono-api/src/index.ts` (Phase 3, Step 4)

- **DATABASE_URL** - Indirectly via KoanService
- **HONO_API_PORT** - Server port configuration
- **NODE_ENV** - Environment mode (development/production)

#### `packages/services/KoanService.ts` (Phase 4, Step 1)
...
| ------------------ | -------------------------- | -------- |
| `DATABASE_URL`     | All database-related files | Phase 1+ |
| `EXPRESS_API_PORT` | Express API server         | Phase 2  |
| `HONO_API_PORT`    | Hono API server            | Phase 3  |
| `NODE_ENV`         | API servers                | Phase 2+ |
| `REDIS_URL`        | Service layer (caching)    | Phase 4  |
